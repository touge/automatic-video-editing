...
from src.core.frame_accurate_video_composer_v2 import FrameAccurateVideoComposerV2
...
class VideoGenerator:
    ...
    def _run_frame_accurate_composition(self, force_rerun: bool = False) -> str:
        ...
        if force_rerun:
            log.warning("Force rerun is enabled. Deleting existing video files to ensure a fresh composition.")
        ...
        if os.path.exists(output_path):
            log.info(f"Base video with audio already exists: {output_path}")
            return output_path
        ...
        composer = FrameAccurateVideoComposerV2(
    ...
    # def run(self, stage: str, burn_subtitle: bool, force_rerun: bool = False):
    def run(self, stage: str, force_rerun: bool = False):
        """
        Step 2: Runs the final video assembly.
        """
        # log.info(f"--- Starting Final Assembly for Task ID: {self.task_manager.task_id} (Burn Subtitle: {burn_subtitle}, Force Rerun: {force_rerun}) ---")
        log.info(f"--- Starting Final Assembly for Task ID: {self.task_manager.task_id} ( Force Rerun: {force_rerun}) ---")

        # 1. 始终调用新引擎生成带音频的基准视频 (video_with_audio.mp4)
        video_with_audio_path = self._run_frame_accurate_composition(force_rerun=force_rerun)
