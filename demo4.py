...
def trim_segment(src, dur, dst, video_cfg, codec, extra, hwaccel, debug):
    """
    裁剪 src 的前 dur 秒并转码到 dst。
    尝试顺序：硬件编码 -> libx264 -> 流拷贝。
    失败时跳过该片段，不抛出异常（非 debug 模式）。
    """
    vf = (
    ...
    # 尝试编码函数
    def try_encode(cmd, mode_desc):
        try:
            run_cmd(cmd, debug)
            return True
        except CalledProcessError as e:
            if debug:
                print(f"[DEBUG] {mode_desc} 失败，命令：{' '.join(cmd)}", file=sys.stderr)
            else:
                print(f"警告：{mode_desc} 失败，正在尝试下一个方案", file=sys.stderr)
            return False
    ...
    if try_encode(cmd3, "流拷贝裁剪"):
        return

    # 4) 全部失败
    msg = f"ERROR: 无法处理片段 {src}，已跳过"
    if debug:
