...
def trim_segment(src, dur, dst, video_cfg, codec, extra, hwaccel, debug):
    """
    裁剪 src 的前 dur 秒，并按 video_cfg 应用 scale/pad/fps，
    使用硬件编码器失败时回退到 libx264。
    """
    vf = (
        f"scale={video_cfg['width']}:-2:force_original_aspect_ratio=decrease,"
        f"pad={video_cfg['width']}:{video_cfg['height']}:(ow-iw)/2:(oh-ih)/2:color=black,"
        f"fps={video_cfg['fps']}"
    )
    base_cmd = ['ffmpeg', '-y']
    ...
    try:
        run_cmd(base_cmd, debug)
    except CalledProcessError:
        # 如果已是 libx264，跳过并在 debug 模式下抛出
        if codec == 'libx264':
            if debug:
                raise
            print(f"警告：libx264 转码失败，跳过 {src}", file=sys.stderr)
            return
        # 回退到 libx264
        print(f"警告：硬件编码器 {codec} 失败，改用 libx264", file=sys.stderr)
        sw_cmd = base_cmd.copy()
        idx = sw_cmd.index('-c:v')
        sw_cmd[idx+1] = 'libx264'
        # 插入 veryfast preset
        sw_cmd.insert(idx+2, '-preset')
        sw_cmd.insert(idx+3, 'veryfast')
        run_cmd(sw_cmd, debug)

def make_concat_list(segments, list_path):
    """
    生成 concat demuxer 的 list.txt
    """
...
def concat_with_progress(
    ...
    """
    拼接视频并实时显示进度。debug=True 时同步执行并输出日志。
    """
    base = [
        'ffmpeg', '-y',
        '-fflags', '+genpts',
...
def merge_audio_with_progress(
    ...
    """
    合并外部音频并显示进度。debug=True 时同步执行并输出日志。
    """
    base = [
        'ffmpeg', '-y',
        '-i', str(video_mp4),
...
def main():
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix='vid_all_', dir=str(temp_root)))
